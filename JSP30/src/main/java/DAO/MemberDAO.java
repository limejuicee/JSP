...
public class MemberDAO {
	...
	//필드를 만들어봐야 생성자를 마았으니 사용이 불가능
	//생성자가 막혔음에도 해당 필드의 내용을 사용할 수 있도록 static타입으로 변경
	//static타입의 변수에 객체를 담았으니 해당 객체는 1개를 초과하여 존재할 수 없다
	...
	public Member getMember(String userid) {
		...
		try {
			...
			if(rs.next()) {		//resultset이 empty set이 아니라면 한줄을 읽고 {}내부를 실행
				m=new Member();	//private타입 변수 7개와 getset 메서드 14개로 이루어진 객체 생성
	...
	public int userCheck(String userid, String pwd) {
		...
		try {
			...
			pstmt.executeQuery();				//pstmt는 실행시에는 sql을 별도로 주지 않는다
			rs=pstmt.executeQuery();
			if(rs.next()) {
				//select문의 결과가 있는 경우
	...
	public int insertMember(Member m) {
		...
		try {
			...
			//executeQuery() : return boolean - insert문에 사용, ResultSet의 존재여부
			//executeUpdate() : return int - update, delete문에 사용, 변동된 줄의 수
		}catch(Exception e) {
			System.out.println("회원가입(insertMember(Member) 접속 중 오류발생 : "+e);
		}finally {
			try {
				if(pstmt!=null)pstmt.close();
				if(conn!=null)conn.close();
	...
	public int updateMember(Member m) {
		int result=-1;	//-1정보수정 실패, 그이외에는 정보가 수정된 줄 수
		Connection conn=null;
		PreparedStatement pstmt=null;
		...
		try {
			...
			System.out.println("updateMember(Member m)접속중 오류 발생 : "+e);
		}finally {
			try {
				if(pstmt!=null)pstmt.close();
				if(conn!=null)conn.close();				
